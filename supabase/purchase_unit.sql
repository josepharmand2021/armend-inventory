-- ARMEND — purchase unit / pack size + loss factor.
--   * cost per base unit is derived from how you buy the item (box, botol, ctn)
--   * loss_pct inflates a recipe's usage AND its HPP (expected waste/yield loss)
-- Run once in the Supabase SQL Editor. Safe to re-run.

alter table public.items add column if not exists purchase_unit text;                        -- ctn, botol, pack, …
alter table public.items add column if not exists pack_size numeric not null default 0;       -- base units per purchase unit
alter table public.items add column if not exists purchase_cost numeric not null default 0;   -- Rp per purchase unit
alter table public.items add column if not exists loss_pct numeric not null default 0;        -- expected loss/waste factor %

-- when both purchase fields are set, cost_per_unit is computed
...
$$;
drop trigger if exists trg_items_derive_cost on public.items;
create trigger trg_items_derive_cost
  before insert or update on public.items
  for each row execute procedure public.items_derive_cost();

-- keep the new fields admin-only alongside the rest of the item master
